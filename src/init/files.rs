...
pub async fn init_manifest(root: &'_ Path, url: &Option<String>, statement: Option<Statement>) -> R<()> {
	let manifest = crate::manifest::Manifest { task_url: url.clone(), statement };
	manifest.save(root).await?;
	Ok(())
}

pub async fn init_template(root: &'_ Path) -> R<()> {
...
pub async fn init_examples(root: &'_ Path, examples: &[Example]) -> R<()> {
