use crate::{
	dir, init::SOLUTION_TEMPLATE, util::{fs, path::PathRef}
};
use evscode::{error::ResultExt, R};
use unijudge::{Example, Statement};

pub async fn init_manifest(root: PathRef<'_>, url: &Option<String>, statement: Option<Statement>) -> R<()> {
	let manifest = crate::manifest::Manifest { task_url: url.clone(), statement };
	manifest.save(root).await?;
	Ok(())
}

pub async fn init_template(root: PathRef<'_>) -> R<()> {
...
pub async fn init_examples(root: PathRef<'_>, examples: &[Example]) -> R<()> {
