...
impl Library {
	pub fn load(path: &Path) -> R<Library> {
		let file = File::open(path).context(format!("library not found in {}", path.display()))?;
	...
	fn verify(&self) -> R<()> {
		for (_, piece) in &self.pieces {
			if let Some(parent) = &piece.parent {
				if !self.pieces.contains_key(parent) {
					return Err(error::Category::MalformedLibrary { detail: "parent does not exist" }.err())?;
				}
				if self.pieces[parent].parent.is_some() {
					return Err(error::Category::MalformedLibrary {
						detail: "doubly nested library pieces are not supported yet",
					}
					.err())?;
				}
			}
		...
		if og.toposort().is_none() {
			return Err(error::Category::MalformedLibrary {
				detail: "ordering cycle detected",
			}
			.err())?;
		}
		Ok(())
	...
	pub fn place(&self, piece_id: &str, source: &str) -> R<(vscode::Position, String)> {
